...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/23 17:10:54 by nryser            #+#    #+#             */
/*   Updated: 2025/04/23 17:11:08 by nryser           ###   ########.ch       */
/*                                                                            */
/* ************************************************************************** */
...
t_colour	stripe_at(t_pattern *pattern, t_tuple point, t_object *obj)
{
	...
	if (stripe_index % 2 == 0)
	{
		if (pattern->a_pattern)
			return (pattern_colour_at(pattern->a_pattern, point, obj));
		return (pattern->a);
	}
	else
	{
		if (pattern->b_pattern)
			return (pattern_colour_at(pattern->b_pattern, point, obj));
		return (pattern->b);
	}
}
