...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/29 16:44:17 by nryser            #+#    #+#             */
/*   Updated: 2025/04/29 16:46:22 by nryser           ###   ########.ch       */
/*                                                                            */
/* ************************************************************************** */
...
int	rodrigues_rotation_test(int run)
{
	...
	#define TEST_RODRIGUES(FROM, TO, LABEL) { \
		...
		if (fabs(rotated.x - to.x) > 0.01 || fabs(rotated.y - to.y) > 0.01 || fabs(rotated.z - to.z) > 0.01) \
	...
	printf("First test #####################################################\n");
	TEST_RODRIGUES(ft_tuple(0, 1, 0, VECTOR), ft_tuple(1, 0, 0, VECTOR), "Rotate UP → RIGHT");

	printf("Second test #####################################################\n");
	TEST_RODRIGUES(ft_tuple(0, 0, 1, VECTOR), ft_tuple(0, 1, 0, VECTOR), "Rotate FORWARD → UP");

	printf("Third test #####################################################\n");
	TEST_RODRIGUES(ft_tuple(1, 0, 0, VECTOR), ft_tuple(0, 0, -1, VECTOR), "Rotate RIGHT → BACKWARD");
