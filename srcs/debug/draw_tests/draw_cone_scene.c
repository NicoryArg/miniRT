...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/28 03:18:52 by nryser            #+#    #+#             */
/*   Updated: 2025/04/28 03:18:52 by nryser           ###   ########.ch       */
/*                                                                            */
/* ************************************************************************** */
...
t_world	*scene_icecream(void)
{
	...
	t_pattern	*floor_pat, *wall_pat_left, *wall_pat_right;
	t_pattern	*pat_col1, *pat_col2, *roof_pat, *cone_pat, *scoop1_pat, *scoop2_pat, *scoop3_pat;
	...
	*floor_pat = uv_checkers_pattern(ft_colour(0.5, 0, 0.9), ft_colour(0, 0.8, 0.8), 2, 2);
	...
	*wall_pat_left = uv_gradient_pattern(ft_colour(0.5, 0, 0.9), ft_colour(0, 0.8, 0.8), 0, 0);
	wall_pat_left->transform = multiply_matrices(rotate_y(-M_PI/4), multiply_matrices(rotate_x(M_PI/2), scale(1, 1, 1)));
	...
	*wall_pat_right = uv_stripe_pattern(ft_colour(0.5, 0, 0.9), ft_colour(0, 0.8, 0.8), 10, 1);
	wall_pat_right->transform = multiply_matrices(rotate_y(M_PI/4), multiply_matrices(rotate_x(M_PI/2), scale(1, 1, 1)));
	...
	pat_col1 = malloc(sizeof(t_pattern));
	*pat_col1 = uv_checkers_pattern(ft_colour(0, 0, 0.7), ft_colour(0, 0.3, 0.7), 6, 6);
	pat_col1->transform = create_identity_matrix(4);
	house_col1->base.m.pattern = pat_col1;
	house_col1->min = 0;
	house_col1->max = 2.5;
	...
	pat_col2 = malloc(sizeof(t_pattern));
	*pat_col2 = uv_checkers_pattern(ft_colour(0, 0, 0.7), ft_colour(0, 0.3, 0.7), 6, 6);
	pat_col2->transform = create_identity_matrix(4);
	house_col2->base.m.pattern = pat_col2;
	house_col2->min = 0;
	house_col2->max = 2.5;
	...
	roof_pat = malloc(sizeof(t_pattern));
	*roof_pat = uv_checkers_pattern(ft_colour(0.8, 0.3, 0.1), ft_colour(0.9, 0.6, 0.3), 10, 10);
	roof_pat->transform = create_identity_matrix(4);
	roof_cone->base.m.pattern = roof_pat;
	...
	cone_pat = malloc(sizeof(t_pattern));
	*cone_pat = uv_checkers_pattern(ft_colour(0.9, 0.6, 0.3), ft_colour(0.9, 0.5, 0.2), 35, 35);
	cone_pat->transform = create_identity_matrix(4);
	ice_cone->base.m.pattern = cone_pat;

	// ice_cone->base.m.c = ft_colour(0.9, 0.6, 0.3);
	ice_cone->min = 0;
	ice_cone->max = 2;
	ice_cone->closed = false;
	transform = multiply_matrices(scale(0.5, 1.0, 0.5), translate(-1.3, -0.2, -4.5));
	...
	scoop1_pat = malloc(sizeof(t_pattern));
	*scoop1_pat = uv_checkers_pattern(ft_colour(1, 0.2, 1), ft_colour(1, 1, 0), 6, 6);
	scoop1_pat->transform = create_identity_matrix(4);
	scoop1->base.m.pattern = scoop1_pat;
	transform = multiply_matrices(scale(0.4, 0.4, 0.4), translate(-1.5, 4.6, -8.5));
	...
	scoop2_pat = malloc(sizeof(t_pattern));
	*scoop2_pat = ring_pattern(ft_colour(0.3, 1, 0.3), ft_colour(0.9, 0.9, 0.3));
	scoop2_pat->transform = create_identity_matrix(4);
	scoop2->base.m.pattern = scoop2_pat;
	transform = multiply_matrices(scale(0.4, 0.4, 0.4), translate(-3.5, 4.6, -8.5));
	...
	scoop3_pat = malloc(sizeof(t_pattern));
	*scoop3_pat = uv_stripe_pattern(ft_colour(0.8, 0.8, 1), ft_colour(1, 0.7, 0.7), 2, 2);
	scoop3_pat->transform = create_identity_matrix(4);
	scoop3->base.m.pattern = scoop3_pat;
	transform = multiply_matrices(scale(0.4, 0.4, 0.4), translate(-2.5, 6.3, -8.5));
