...
t_world	*scene_pattern(void)
{
	...
	*pat1 = uv_checkers_pattern(ft_colour(1, 0.2, 1), ft_colour(1, 1, 0), 12, 12);
	...
	*pat3 = gradient_pattern(ft_colour(1, 0.8, 1), ft_colour(1, 0.2, 1));
	...
	*patcyl1 = uv_checkers_pattern(ft_colour(1, 0, 0), ft_colour(0, 1, 0), 20, 6);
	...
	cyl1->max = 8;
	cyl1->closed = true;
	transform =multiply_and_free(rotate_y(M_PI / 6) , multiply_and_free(scale(0.5, 1.0, 0.5), translate(1.2, 0.3, 1.5)));
	...
	*patcyl2 = uv_gradient_pattern(ft_colour(0.2, 0.2, 1), ft_colour(1, 1, 0), 0, 0);
	...
	cyl2->max = 5;
	cyl2->closed = true;
	transform = multiply_and_free(scale(0.5, 1.0, 0.5), multiply_and_free(translate(-4.5, 0, -0.5), rotate_x(M_PI / 3)));
	...
	*patcyl3 = uv_stripe_pattern(ft_colour(1, 0.2, 1), ft_colour(0, 1, 1), 10, 1);
	...
	cyl3->max = 2;
	cyl3->closed = true;
	transform =multiply_and_free(rotate_y(M_PI / 4) , multiply_and_free(scale(0.3, 0.3, 0.3),
		multiply_and_free(translate(1.5, 1.3, -12.8), multiply_and_free(rotate_x(M_PI / 2), rotate_z(M_PI / 4)))));
