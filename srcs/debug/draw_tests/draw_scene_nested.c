...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/20 18:25:28 by nryser            #+#    #+#             */
/*   Updated: 2025/04/20 18:27:50 by nryser           ###   ########.ch       */
/*                                                                            */
/* ************************************************************************** */
...
//Floor
	floor = ft_plane();
	t_pattern *nested_checkers = malloc(sizeof(t_pattern));
	nested_checkers->type = PATTERN_CHECKERS;
	nested_checkers->frequency = 4.0;
	nested_checkers->a_pattern = make_diagonal_stripe(ft_colour(1, 0.2, 1), ft_colour(0, 0, 0), M_PI / 4);
	nested_checkers->b_pattern = make_diagonal_stripe(ft_colour(1, 1, 0), ft_colour(0, 0, 0), -M_PI / 4);
	nested_checkers->transform = create_identity_matrix(4); // no transform

	floor->base.m.pattern = nested_checkers;
