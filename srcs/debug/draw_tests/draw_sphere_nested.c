...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/20 17:24:06 by nryser            #+#    #+#             */
/*   Updated: 2025/04/20 17:25:08 by nryser           ###   ########.ch       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_sphere_nested(t_engine *engine)
{
	...
	// 🧩 Create nested pattern: UV checkers with diagonal UV stripes
	checkers = malloc(sizeof(t_pattern));
	checkers->type = PATTERN_CHECKERS;
	checkers->width = 8;
	checkers->height = 8;
	checkers->a_pattern = make_diagonal_stripe(ft_colour(0, 1, 0), ft_colour(0, 0, 0), M_PI / 4);
	checkers->b_pattern = make_diagonal_stripe(ft_colour(0, 1, 1),
			ft_colour(0, 0, 0), -M_PI / 4);
	checkers->transform = create_identity_matrix(4);
	ctx.sph->base.m.pattern = checkers;
