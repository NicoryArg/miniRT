...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/29 20:04:24 by nryser            #+#    #+#             */
/*   Updated: 2025/04/29 20:04:30 by nryser           ###   ########.ch       */
/*                                                                            */
/* ************************************************************************** */
...
t_pattern *make_uv_pattern(t_pattern (*fn)(t_colour, t_colour, int, int),
...
t_plane *make_wall(double x_offset, double y_rot_angle, t_pattern *pat)
{
	t_plane *wall = ft_plane();
	wall->base.m = ft_material();
	wall->base.m.specular = 0;
	wall->base.m.pattern = pat;
	t_matrix *rot = multiply_matrices(rotate_y(y_rot_angle), rotate_x(M_PI / 2));
	set_transf(wall, multiply_matrices(translate(x_offset, 0, 0), rot));
	free_matrix(rot);
	return wall;
}

t_cylinder *make_column(t_colour a, t_colour b, double tx, double ty, double tz)
{
	t_cylinder *col = ft_cylinder();
	col->base.m = ft_material();
	col->base.m.pattern = make_uv_pattern(uv_checkers_pattern, a, b, 6, 6);
	col->min = 0;
	col->max = 2.5;
	col->closed = true;
	t_matrix *m = multiply_matrices(scale(0.3, 1.5, 0.3), translate(tx, ty, tz));
	set_transf(col, m);
	free_matrix(m);
	return col;
}
					  ...
					  t_matrix *scale_m, t_matrix *trans_m)
{
	...
	cone->base.m.pattern = make_uv_pattern(uv_checkers_pattern, a, b, check_w, check_h);
	cone->min = min;
	cone->max = max;
	cone->closed = closed;
...
t_sphere *make_scoop(t_pattern *pat, double sx, double sy, double sz, double tx, double ty, double tz)
{
	t_sphere *s = ft_sphere(1);
	s->base.m = ft_material();
	s->base.m.pattern = pat;
...
static t_pattern	*make_pattern(t_pattern (*func)(t_colour, t_colour), t_colour a, t_colour b)
{
	t_pattern *pat = malloc(sizeof(t_pattern));
	*pat = func(a, b);
	pat->transform = create_identity_matrix(4);
...
static void	set_object(t_object *obj, t_pattern *pat, t_matrix *tr)
{
	obj->m = ft_material();
	obj->m.pattern = pat;
	set_transf(obj, tr);
...
t_world	*scene_idea(void)
{
	...
	set_object(&floor->base, make_uv_pattern(uv_checkers_pattern,
		ft_colour(0.5, 0, 0.9), ft_colour(0, 0.8, 0.8), 2, 2),
		create_identity_matrix(4));
	floor->base.m.specular = 0;
	add_object(objs, &floor->base, &idx);

	t_matrix *left_rot = multiply_matrices(rotate_y(-M_PI / 4), rotate_x(M_PI / 2));
	t_plane *left_wall = ft_plane();
	set_object(&left_wall->base, make_uv_pattern(uv_gradient_pattern,
	...
	t_matrix *right_rot = multiply_matrices(rotate_y(M_PI / 4), rotate_x(M_PI / 2));
	t_plane *right_wall = ft_plane();
	set_object(&right_wall->base, make_uv_pattern(uv_stripe_pattern,
	...
	set_object(&col1->base, make_uv_pattern(uv_checkers_pattern,
	...
	set_object(&col2->base, make_uv_pattern(uv_checkers_pattern,
		ft_colour(0, 0, 0.7), ft_colour(0, 0.3, 0.7), 6, 6),
	...
	set_object(&roof->base, make_uv_pattern(uv_checkers_pattern,
	...
	set_object(&ice_cone->base, make_uv_pattern(uv_checkers_pattern,
	...
	set_object(&s1->base, make_uv_pattern(uv_checkers_pattern,
	...
	set_object(&s2->base, make_pattern(ring_pattern,
	...
	set_object(&s3->base, make_uv_pattern(uv_stripe_pattern,
